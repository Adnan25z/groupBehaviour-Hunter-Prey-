...
class Agent(object):
    def __init__(self, world=None, scale=30.0, mass=1.0, mode='seek', is_predator=False):
    ...
    def calculate(self, delta):
        ...
        # If the agent is a predator, use the wander behavior
        if self.is_predator:
            return self.wander(delta)
        # If the agent is not a predator, consider the predator's presence
        predator = self.get_predator()
        if predator:
            distance_to_predator = (predator.pos - self.pos).length()
            flee_radius = 200  # Adjust this value as needed
            if distance_to_predator < flee_radius:
                return self.flee(predator.pos)
    ...
    def update(self, delta):
        ...
        # update emerge and separate lists
    ...
    def flee(self, target_pos):
        ...
        desired_vel = (self.pos - target_pos).normalise() * self.max_speed
        return (desired_vel - self.vel)
